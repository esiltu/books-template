...
const VerifyPhoneVerification = ({ route }) => {
  ...
  const phoneNumber =
    route?.params?.phoneNumber || navigation?.getParam({ phoneNumber });
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "white" }}>
      ...
      <View style={{ top: "2.5%" }}>
        ...
        <Text style={styles.subtitle}>
          Please enter the code we just send to the phone number{" "}
      ...
      <View style={{ alignSelf: "center", top: "5%" }}>
        <TouchableOpacity
          ...
          activeOpacity={0.6}
        >
          <Text style={{ fontSize: 15, color: "#A6A6A6", fontWeight: "600" }}>
            If you didn't receive a code?{" "}
            <Text style={{ color: "#54408C" }}>Resend</Text>
          </Text>
        </TouchableOpacity>
      </View>
